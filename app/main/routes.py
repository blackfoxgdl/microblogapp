...
def search():
	...
	page = request.args.get('page', 1, type=int)
	posts, total = Post.search(g.searchForm.q.data, page, current_app.config['POST_PER_PAGE'])
	next_url = url_for('main.search', q=g.searchForm.q.data, page=page + 1) \
				if total > page * current_app.config['POST_PER_PAGE'] else None
	prev_url = url_for('main.search', q=g.searchForm.q.data, page=page -1) \
				if total > 1 else None

	render_template('search.html', title=_('Search'), posts=posts, next_url=next_url, prev_url=prev_url)
